...
export interface ProjectObjective {
  ...
  completed: boolean;
  completedAt?: string;
  keyResults?: ProjectKeyResult[]; // 목업 데이터 호환성을 위해 추가
}
...
export interface ProjectKeyResult {
  ...
  currentValue: number;
  unit: string;
  progress: number; // 목업 데이터 호환성을 위해 추가
  completed?: boolean; // 선택적으로 변경
  completedAt?: string;
}
...
export interface ProjectMember {
  id: string;
  projectId?: string; // 선택적으로 변경
  userId: string;
  role: ProjectMemberRole;
  permissions?: string[]; // 선택적으로 변경
  joinedAt: string;
  user: ProjectUser;
}
...
export interface Project {
  ...
  tasks?: ProjectTask[]; // Task 타입
  goal?: ProjectGoal; // Goal 타입
...
export interface ProjectDashboard {
  ...
  recentTasks: ProjectTask[]; // Task 타입
  upcomingMilestones: {
    id: string;
    title: string;
    dueDate: string;
